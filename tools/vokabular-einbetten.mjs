...
const datenOrdner = join(wurzel, 'prototype', 'data');
...
/*
 * Der Beispieldatensatz folgt demselben Muster: die .json ist die Quelle, die
 * .js macht sie fuer den Doppelklick-Betrieb lesbar. Sonst koennte
 * auswertung.html offline nichts anzeigen.
 */
function einbettenDaten(daten) {
  return [
    '/*',
    ...
    'window.EduStandard.beispieldaten = ' + JSON.stringify(daten, null, 2) + ';',
    ''
  ].join('\n');
}
...
for (const name of (await readdir(datenOrdner)).filter((n) => n.endsWith('.json')).sort()) {
  const daten = JSON.parse(await readFile(join(datenOrdner, name), 'utf8'));
  const ziel = join(datenOrdner, basename(name, '.json') + '.js');
  const inhalt = einbettenDaten(daten);
  if (nurPruefen) {
    let vorhanden = null;
    try { vorhanden = await readFile(ziel, 'utf8'); } catch { /* fehlt */ }
    if (vorhanden !== inhalt) { abweichungen += 1; console.error('abweichend: ' + basename(ziel)); }
  } else {
